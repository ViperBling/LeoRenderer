...
struct ImGui_ImplVulkan_Data
{
    ...
    ImVector<VkDescriptorPool>  DescriptorPools;
...
static void ImGui_ImplVulkan_ResetDescriptorPool(VkDescriptorPool pool)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    VkResult err;

    err = vkResetDescriptorPool(v->Device, pool, 0);
    check_vk_result(err);
}

static VkDescriptorSet ImGui_ImplVulkan_AllocateDescriptorSet(ImTextureID texture)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    VkResult err;
    ...
    // Create Descriptor Set:
    {
        ...
        alloc_info.descriptorPool = bd->DescriptorPools[bd->InFlyFrameIndex];
    ...
    // Update the Descriptor Set:
    {
        ...
        desc_image[0].imageView = (VkImageView)texture;
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                VkDescriptorSet descriptorSet = ImGui_ImplVulkan_AllocateDescriptorSet(pcmd->TextureId);
...
static void ImGui_ImplVulkan_CreateSampler(VkDevice device, const VkAllocationCallbacks* allocator)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    if (bd->Sampler)
        return;
    ...
    VkResult err = vkCreateSampler(device, &info, allocator, &bd->Sampler);
    check_vk_result(err);
}
...
static void ImGui_ImplVulkan_CreateDescriptorSetLayout(VkDevice device, const VkAllocationCallbacks* allocator)
{
    ...
    ImGui_ImplVulkan_CreateSampler(device, allocator);
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
    ...
    if (bd->DescriptorPools.Size == 0)
    {
        ...
        info.maxSets = 2048;
        info.pPoolSizes = descriptorPoolSizes;
        info.poolSizeCount = IM_ARRAYSIZE(descriptorPoolSizes);

        bd->DescriptorPools.resize(v->InFlyFrameCount);
        for (int i = 0; i < bd->DescriptorPools.Size; i++)
        {
            err = vkCreateDescriptorPool(v->Device, &info, v->Allocator, &bd->DescriptorPools[i]);
            check_vk_result(err);
        }
    }
    ...
    if (!bd->Sampler)
    {
        ...
        info.maxLod = 1000;
        info.maxAnisotropy = 1.0f;
        err = vkCreateSampler(v->Device, &info, v->Allocator, &bd->Sampler);
        check_vk_result(err);
    }

    if (!bd->DescriptorSetLayout)
    {
        VkSampler sampler[1] = {bd->Sampler};
        VkDescriptorSetLayoutBinding binding[1] = {};
        binding[0].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        binding[0].descriptorCount = 1;
        binding[0].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
...
void    ImGui_ImplVulkan_DestroyDeviceObjects()
{
    ...
    for (int i = 0; i < bd->DescriptorPools.Size; i++)
    {
        if (bd->DescriptorPools[i]) { vkDestroyDescriptorPool(v->Device, bd->DescriptorPools[i], v->Allocator); bd->DescriptorPools[i] = VK_NULL_HANDLE; }
    }
    ...
    if (bd->Sampler)              { vkDestroySampler(v->Device, bd->Sampler, v->Allocator); bd->Sampler = VK_NULL_HANDLE; }
...
bool    ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info, VkRenderPass render_pass)
{
    ...
    IM_ASSERT(info->InFlyFrameCount >= 1);
...
void ImGui_ImplVulkan_NewFrame()
{
    ...
    bd->InFlyFrameIndex = (bd->InFlyFrameIndex + 1) % v->InFlyFrameCount;
    ImGui_ImplVulkan_ResetDescriptorPool(bd->DescriptorPools[bd->InFlyFrameIndex]);
}
